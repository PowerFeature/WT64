...
#define USE_LUMA 0
#define BORDER_COLOR float4(0.647f, 0.647f, 1, 1)
#define BACKGROUND_COLOR float4(0.259f, 0.259f, 0.906f, 1)
...
float2 transformCurve(float2 uv) {
  ...
  float r = uv.x * uv.x * CURVE_FACTOR + uv.y * uv.y * CURVE_FACTOR; 	// get ratio
  uv *= 4.65f + r;				// apply ratio
  uv *= 0.230f;				// zoom
...
float4 main(PSInput pin) : SV_TARGET {
  ...
  //if(uv.x > 1.025f  || uv.y > 1.025f)  return float4(0.3f, 0.3f, 0.3f, 1.f); 
  //if(uv.x < -0.055f || uv.y < -0.055f)  return BORDER_COLOR;
  //if(uv.x > 1.055f  || uv.y > 1.055f)   return BORDER_COLOR;
  // Bezel
  //if(uv.x < -0.015f || uv.y < -0.015f) return BORDER_COLOR;
  //if(uv.x > 1.015f  || uv.y > 1.015f)  return BORDER_COLOR;
  // Screen Border
  
  if(uv.x < 0.000f  || uv.y < 0.000f)  return BORDER_COLOR;
  if(uv.x > 1.000f  || uv.y > 1.000f)  return BORDER_COLOR;
  #endif
